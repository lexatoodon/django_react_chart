...
class UnfinishedOrders extends Orders {
    ...
    componentDidMount() {
        fetch(
          "http://localhost:8000/chart/api/unfinished/")
            .then((res) => res.json())
            .then((json) => {
                this.setState({
                    items: json,
    ...
    render() {
        ...
        return (
        <div className = "List">
            <p>Total number of unfinished orders: <strong>{items.length}</strong></p>
            <p>Estimated income:
            ...
            {chart}
            <h5>List of unfinished orders:</h5>
            {items.map((item) => (
